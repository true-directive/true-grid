...
@Component({
  selector: 'true-editor-selector',
  template: `<div [ngClass]="getClass()">
              <true-selector #input
                class="true-editor-selector__selector"
                [(ngModel)]="value"
                [style.height]="getH()"
                ...
                (keydown)="inputKeyDown($event)"></true-selector>
            </div>`,
  styleUrls: ['./editor-selector.component.scss']
  })
export class EditorSelectorComponent implements IEditor, OnDestroy {
  ...
  get settings() {
    if (this._settings === null) {
      ...
      this._settings.commonFilterDelay = 100;
    }
  ...
  get items(): any[] {
    if (this._items === null) {
      if (this.column.optionsDataSource instanceof Observable) {
        // Подпись
        const observable = <Observable<any>>this.column.optionsDataSource;
        observable.pipe(takeUntil(this.destroy$)).subscribe(data => {
          this._items = data;
        });
      } else {
        this._items = this.column.optionsDataSource;
      }
    }
    return this._items;
  }
