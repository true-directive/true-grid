...
@Component({
  ...
  template:`
      ...
      <true-popup #popup
        ...
        <true-grid #grid
                 [class.true-selector__grid_hidden]="empty()"
                 class="true-selector__grid"
        ...
        <div *ngIf="empty()" class="true-selector__notfound" [style.width]="gridWidth()">Ничего не найдено</div>
        <div *ngIf="updating()" class="true-selector__updating">Загрузка...</div>
      </true-popup>
    `,
  styles: [`
    :host {
    ...
    .true-selector__notfound, .true-selector__updating {
      padding: 0.6em;
      box-sizing: border-box;
      color: #aaa;
    }
...
export class SelectorComponent extends DropdownBaseComponent {
  ...
  gridEndProcess(e: any) {
    setTimeout(() => {
      if (!this.popup.visible) {
        ...
        // Обновим позицию. При уменьшении списка выпадающее окно может быть
        // перемещено относительно от инпута.
        // Let's update position of popup window. When list reduce, it may move up or down regarding input.
  ...
  public inputInput(e: any = null) {
    ...
    if (!this.popupVisible && this.displayValue !== '') {
      this.grid.commonFilter = this.displayValue;
      return;
    }
