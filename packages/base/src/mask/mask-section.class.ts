...
export class MaskSection {
  ...
  public autoCorrectVal(s: string): string {
    ...
    if (this.hasOptions()) {
      const variant = this.sectionType.options.find(v => v.toLowerCase() === s.toLowerCase());
      if (!variant) {
  ...
  private skip(mv: MaskValue, selStart: number): MaskResult {
    const res: MaskResult = new MaskResult(mv.value(), MaskSectionAction.SKIP, mv.nextSectionPos());
    res.selStart = selStart;
    return res;
  }

  private none(mv: MaskValue): MaskResult {
    const res: MaskResult = new MaskResult(mv.value(), MaskSectionAction.NONE, mv.nextSectionPos());
    return res;
  }
  ...
  private apply(mv: MaskValue, newSectionValue: string, selStart: number, direction: number = 1, isLast: boolean = false): MaskResult {
    ...
    const selStart_local = selStart - mv.sectionPos;
    ...
    if (direction > 0) {
      ...
      if (direction < 0) {
        ...
        // Stand still
        const res: MaskResult = new MaskResult(mv.value(), MaskSectionAction.APPLY, mv.nextSectionPos());
        res.selStart = selStart;
  ...
  private applyDelimiter(mv: MaskValue, selStart: number): MaskResult {
    ...
    // Moving forward
    const res: MaskResult = new MaskResult(mv.value(), MaskSectionAction.GO_FWD, mv.nextSectionPos());
  ...
  private goBack(mv: MaskValue, selStart: number, selLength: number, byBackspace: boolean = false, isLast: boolean = false): MaskResult {
    ...
    const selStart_local = res.selStart - mv.sectionPos;
    ...
    const newSelStart_local = res.selStart - mv.sectionPos;
  ...
  private goFwd(mv: MaskValue, selStart: number, selLength: number, appendDelimiter: boolean = false): MaskResult {
    const selStart_local: number = selStart - mv.sectionPos;
    const res: MaskResult = new MaskResult(mv.value(), MaskSectionAction.APPLY, mv.nextSectionPos());
